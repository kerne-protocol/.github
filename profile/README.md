...
| [**solhonesty**](https://github.com/kerne-protocol/solhonesty) | The same question asked on Solana, across Kamino, Jupiter Lend and Save, published as a [CC BY 4.0 dataset](https://huggingface.co/datasets/kerne-protocol/solana-yield-honesty) and rendered at [kerne.fi/solana-honesty](https://kerne.fi/solana-honesty). Kerne is deliberately **absent** from that board, because the board is Solana and Kerne is on Base, so any Kerne row there would be invented. |
...
- **Tests you cannot run:** the private monorepo holds 2,516 Solidity test functions across 137 test files (measured 2026-07-31) plus Python (bot) and TypeScript (SDK) suites, and a drift-guard CI job that asserts every numeric threshold cited in the docs matches the live constant in code. We list that number for completeness, not as evidence: you cannot execute it, so weigh the 82 above instead. Earlier revisions of this page said "900+ Solidity tests" and left it at that, which was true and understated but gave a reader nothing to check.
